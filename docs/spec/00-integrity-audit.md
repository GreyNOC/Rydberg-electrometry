...
| R20 | MSD94 potential tables + a₄(l=1) choice −0.81633314 | 02 §3.1–3.3 | LOW | Both transcriptions re-fetched and diffed this audit; the single discrepant digit is real, disclosed, and bounded. **Impact bound CORRECTED 2026-08-10** — this row said "< 4×10⁻⁸ effect on Z₁" and spec 02 §3.3 said "δa₄ = 8×10⁻⁸ perturbs …"; neither is the measured figure, and the 8×10⁻⁸ was not even the right δa₄ (the actual transcription gap is 1.74×10⁻⁶ — that mismatch is what exposed the error; spec 02 §3.3 now carries the re-derivation). The reproducible bound is the one shipped in `rydsim.radial.A4_L1_NOTE`, verbatim: *"|delta a4| = 1.74e-6 perturbs Z_1(r) by <= 2.53e-7 (max at r = 1.04 a0)"* — regenerated at test time by `tests/test_radial.py::test_a4_l1_impact_bound_is_reproducible` (which also pins r_peak = 2/a₂ analytically), so the bound cannot go stale inside a provenance string. Still far below all tolerances: adopting the ARC reading instead moves the Rb 50S→50P consensus ME by 6.5×10⁻¹⁴ relative and the reproduced Rb P-series defect by 4.7×10⁻⁸. | Keep the §3.3 note. **The declared mitigation is WITHDRAWN 2026-08-10 — it was empirically false.** "B8 (A-vs-B spread ≤ 10⁻⁴) catches any *material* potential error because Method B is potential-independent" does not hold: everywhere else in `rydsim.radial` the energy is an *input* taken from spec 01's measured defects, so the potential only shapes the orbital inside the divergence-guard radius. Measured: doubling Rb a₃(l=0) moves the 50S→50P A-vs-B spread only 2.02×10⁻⁶ → 2.40×10⁻⁶ against the 10⁻⁴ ceiling (40× inside it; consensus ME moves 4×10⁻⁷), and the a₄(l=1) sign flip moves the ME by 6×10⁻⁸ — i.e. the declared tripwire was **inoperative**, and the one table carrying an UNVERIFIED digit was unguarded. **Working mitigation (shipped):** `rydsim.radial.model_potential_defect` solves MSD94 as a genuine bound-state eigenvalue problem and returns the *predicted* quantum defect — the quantity MSD94 was fitted to, and the one that actually moves — checked by `tests/test_radial.py::test_msd94_reproduces_measured_quantum_defects` against spec 01's measured defects. Measured band: max residual **5.4×10⁻³** across Rb/Cs l = 0…3 centroids (n = 12; worst row Cs P +5.38×10⁻³); guard **1.2×10⁻²** (2.2× the worst residual, 100× the ≤ 1.1×10⁻⁴ numerical wobble). Mutation-tested (`test_msd94_parameter_corruption_is_detected`, 10 corruptions): the same a₃(l=0) doubling shifts the reproduced Rb S defect by +0.41 and the a₄(l=1) sign flip shifts the Rb P centroid by −0.044 — every corruption that B8 passed silently now breaches the band. Anyone with PRA 49, 982 Table I closes the item. |
...
| R24 | Angular-algebra tables: 3j/6j benchmark values, S_FF′ fractions, NIST A-factors, fine-structure ratios | 03 §2, §6; 05 §3.4 | LOW | Pure algebra with dual-implementation (float vs exact-rational) verification; NIST factors are exact radicals (√2/3, √6/5, 2√3/7) matching the published 4-decimal values. The generic large-j float-accuracy *limitation* (8.2×10⁻⁵ at j≈55–70) is itself a disclosed measured result. | Ship the exact-rational oracle in tests (§4.3.6); ~~enforce the rank-1-only certification rule for j > 30~~. **AMENDED 2026-08-10, consequent on refusal #13 being implemented by routing:** the oracle is now shipped in the *library* (`rydsim.wigner.wigner_3j_exact` / `wigner_6j_exact`) and every public symbol falls back to it automatically, so the rank-1-only certification rule no longer exists to enforce — accuracy is ≤ 10⁻¹² by construction at every j and rank. The tests measure and print rather than transcribe: worst relative error vs the oracle **6.6×10⁻¹⁴** over the stratified rank-1 sweep to j = 150 (909 symbols; worst at (42, 1, 43; 0, 0, 0)), **5.4×10⁻¹⁴** over the generic 3j grid (122 symbols) and **2.9×10⁻¹⁴** over the generic 6j grid (36 symbols) — re-measured this pass. The disclosed float-path limitations stand as *historical* measurements of the path that is no longer shipped bare (8.2×10⁻⁵ at j ≈ 55–70; 2.9×10⁻¹¹ at the Δj = 0, \|m\| = 1/2 corner, pinned by `tests/test_wigner.py::test_rank1_worst_case_from_the_audit_sweep`). The live rule is now the declared-domain ceiling j ≤ 150 (refusal #13). |
...
| R26 | Corpus cross-reference pointers: §3.3 says defects live in "docs/spec/03" (they live in 01); C8 attributes the vapor model to "spec 02" (it is 05) | 09 §3.3, §6 C8 | LOW | Pure documentation defects, but they route implementers to the wrong normative tables. | Fix the two pointers in spec 09; CI doc-link check. **RESOLVED 2026-08-10:** both corrected in spec 09 (§3.3 heading and body → spec 01; §6 C8 source → spec 05) per ruling R-13 / the §4 ownership map. CI doc-link check still outstanding. |
...
6. `whittaker_u` for ν > **20** — **`IntegrityError`** (hyperu collapse; re-measure per R4).
   Shipped as `rydsim.radial.WHITTAKER_NU_MAX = 20.0`.
   **AMENDED 2026-08-10 (integrator RULING A), superseding this entry's original text
   "`whittaker_u` for ν > 25 — `ValueError`" and spec 02 §7.3.** Two corrections, both strictly
   tightening the rule:
   - **Fence 25 → 20.** The fence must sit where the method meets its *own* contract: benchmark
     B12 demands 10⁻⁶ pointwise, and 20 < ν ≤ 25 measures up to 5.7×10⁻⁶ — 5.7× outside contract — rising to
     0.49 at ν = 35. (The shipped table, carried in the `whittaker_u` docstring and regenerated on
     the installed scipy by `rydsim.radial.hyperu_hydrogen_error` — reproduced on scipy 1.17.1
     this pass: 6e-13 @ ν=10, 3.5e-8 @ 20, 5.5e-6 @ 25, 1.9e-4 @ 28, 1.8e-3 @ 30, 0.49 @ 35,
     93 @ 40.) `whittaker_u` is a **validation
     instrument only** — nothing on the production Rydberg path calls it (Method B is the
     pure-Coulomb Numerov `coulomb_wavefunction`) — so the fence costs no physics, and a
     validation instrument running outside its claimed accuracy is precisely the "plausible but
     wrong" hazard the house rule exists to stop. A `UserWarning` was judged insufficient for a
     number a cross-check will trust. `rydsim.radial._whittaker_u_unguarded` remains in-module
     for deliberate out-of-contract work (the R4 error table itself).
   - **Exception type.** The original "`ValueError`" was a mis-statement of this register's own
     rule; the code raises `rydsim.provenance.IntegrityError`, which is correct — a refusal
     carrying an integrity reason is an `IntegrityError`. Same for the second refusal
     `whittaker_u` makes (any non-finite `hyperu` return, which on scipy 1.17.1 occurs for
     non-integer ν ≳ 11 *inside* the classically allowed region).
...
13. Generic (non-rank-1) Wigner symbols at j > 30 through the float path — route to the
    ...
    **IMPLEMENTED 2026-08-10 — by the *route to the oracle* arm, not the refusing arm.**
    `rydsim.wigner` no longer ships a bare log-factorial Racah sum. Every public 3j/6j (a) computes
    the float sum together with its own round-off estimate `L·eps/R` (`L` = log-factorial
    magnitude, `R` = cancellation ratio), (b) **recomputes the symbol in exact rational
    arithmetic** (`wigner_3j_exact` / `wigner_6j_exact`, `fractions.Fraction`, no floating point
    until the final square root) whenever that estimate exceeds
    `rydsim.wigner.FLOAT_PATH_TARGET_REL = 1e-12`, and (c) checks the returned value against the
    elementary orthogonality bound (`three_j_bound` / `six_j_bound`), raising `IntegrityError` if
    it is violated. The 10⁻¹² relative accuracy is therefore a **construction** bound holding at
    every j and every rank — there is no j > 30 float-path refusal left to make, and the rank-1-only
    certification caveat of R24 is retired with it. Re-verified this pass: the public
    `wigner_3j(100, 90, 80; 0, 0, 0)` returns −6.783 314 088 538 6×10⁻³, bit-identical to the exact
    oracle, where the bare float path returns +2.52 — wrong sign, 372× too large, and 36× outside
    |3j| ≤ 1/√(2j_max+1) = 0.0705.
    The **remaining refusal is a declared-domain ceiling, not an accuracy one**: every public
    `rydsim.angular` entry point (including its `wigner_3j` / `wigner_6j` wrappers) guards its
    angular momenta at `j ≤ 150` (`rydsim.angular._J_FLOAT_CERT_MAX`) and raises `IntegrityError`
    beyond it, because no physical state in this simulator carries j > 150, so a request past the
    ceiling is a caller bug rather than a physics need. The error message **offers the oracle** as
    the explicit opt-in route ("route through the exact-rational oracle
    `rydsim.angular.wigner_3j_exact` / `wigner_6j_exact` (+ `exact_to_float`), which is exact at
    any j"), and `rydsim.angular` re-exports all three so the route needs no new import.
